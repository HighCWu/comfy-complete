...
def get_output_data(filename, subfolder, file_type):
    """
    ...
    Args:
        ...
        file_type (str): The ComfyUI type ('output' | 'temp').

    Returns:
        tuple: (bytes, content_type) or (None, None) if an error occurs.
    """
    print(
    ...
    try:
        ...
        print(f"worker-comfyui - Successfully fetched {filename} ({content_type})")
...
def handler(job):
    ...
    try:
        ...
        # binary WS frames to the frontend.
                ...
                if isinstance(out, str):
                    ...
                    if msg_type == "status":
                        ...
                        if msg_data.get("prompt_id") == prompt_id:
                            error_details = f"Node Type: {msg_data.get('node_type')}, Node ID: {msg_data.get('node_id')}, Message: {msg_data.get('exception_message')}"
                            print(
        ...
        for node_id, node_output in outputs.items():
            for category in FILE_OUTPUT_CATEGORIES:
                if category not in node_output:
                    continue
                category_files = node_output[category]
                print(
                    f"worker-comfyui - Node {node_id} contains {len(category_files)} {category} output(s)"
                )
                for file_info in category_files:
                    ...
                    file_bytes, content_type = get_output_data(filename, subfolder, file_type)
                    ...
                    if file_bytes:
                        ...
                        if file_type == "temp" and r2_enabled:
                            ...
                            if projected > TEMP_R2_FALLBACK_THRESHOLD:
                                use_r2_for_temp = True

                        if r2_enabled and (
                            file_type == "output" or use_r2_for_temp
                        ) and user_id and input_prompt_id:
                            r2_type = "output" if file_type == "output" else "temp"
                            r2_key = f"{r2_type}/{user_id}/{input_prompt_id}/{filename}"
                            uploaded = upload_to_r2(file_bytes, r2_key, content_type)
                            if uploaded:
                                output_data.append(
                                    {
                                        "filename": filename,
                            ...
                            try:
                                ...
                                print(
                                    f"worker-comfyui - Wrote {category} bytes to temporary file: {temp_file_path}"
                                )
            ...
            # Check for truly unhandled output keys
            handled_categories = set(FILE_OUTPUT_CATEGORIES)
            other_keys = [k for k in node_output.keys() if k not in handled_categories]
            if other_keys:
                warn_msg = (
